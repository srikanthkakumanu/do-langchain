...
def check_python_executable_and_version():
    ...
    if not in_venv:
        ...
        issues.append(
            "   This may cause import errors if required packages are not installed"
        )
        ...
        issues.append(
            "   You may be using a different virtual environment or system Python"
        )
    else:
    ...
    if py_version.major < 3 or (py_version.major == 3 and py_version.minor < 12):
        issues.append(
            f"⚠️  Python {py_version_str} is below minimum required version 3.12"
        )
    elif py_version.major == 3 and py_version.minor >= 14:
        issues.append(
            f"⚠️  Python {py_version_str} is above maximum supported version (< 3.14)"
        )
    else:
        print(
            f"✅ Python version {py_version_str} is in expected range (>=3.12, <3.14)"
        )
...
def check_env_conflicts(env_file_path: str):
    ...
    for key, file_value in env_file_vars.items():
        ...
        if sys_value is not None and sys_value != file_value:
            ...
            conflicts.append(
                {"key": key, "system_value": sys_value, "file_value": file_value}
            )
    ...
    if conflicts:
        ...
        print()
        print(
            "  2. Unset the conflicting system environment variables for this shell session:"
        )
        if sys.platform == "win32":
            print("     CMD:")
            for conflict in conflicts:
...
def doublecheck_env(file_path: str):
    ...
    if langsmith_tracing == "true":
        ...
        if not langsmith_api_key:
            issues.append(
                f"  ⚠️  LANGSMITH_TRACING is enabled but LANGSMITH_API_KEY is not set"
            )
        elif langsmith_api_key == langsmith_example:
            issues.append(
                f"  ⚠️  LANGSMITH_TRACING is enabled but LANGSMITH_API_KEY still has the example/placeholder value"
            )
        else:
    ...
    for dep in deps:
        ...
        try:
            ...
            if not wrong_version:
                if spec not in ("(any)", "(unparsed)") and any(
