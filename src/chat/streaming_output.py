...
# 4. Define a streaming callback handler (must subclass BaseCallbackHandler)
class StreamHandler(BaseCallbackHandler):
    def on_llm_new_token(self, token: str, **kwargs):
        print(token, end="", flush=True)


# 5. Initialize LLMs with streaming enabled
llms = {
    ...
    #     model="gpt-5-nano", temperature=0, streaming=True, callbacks=[StreamHandler()]
    # ),
}

# 6. Create Chain prompt -> model -> parser and Invoke LLMs
